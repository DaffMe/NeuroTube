...
Selain itu, sistem juga menyediakan fitur **CRUD komentar manual** dengan analisis sentimen berbasis kata kunci, serta implementasi algoritma **pencarian** (Sequential & Binary Search) dan **pengurutan** (Selection & Insertion Sort) sebagai pemenuhan persyaratan Tugas Besar.
...
flowchart TD
    ...
    subgraph PyBackend ["ML Tier / Python"]
        ...
        Keyword_Sentiment[Keyword Sentiment Analysis]
        Comment_CRUD[Comment CRUD Endpoints]
    end
    ...
    %% Alur Data - Komentar Manual
    CRUD_UI -- "POST/PUT/DELETE" --> Comment_CRUD
    Comment_CRUD -- "Analisis Kata Kunci" --> Keyword_Sentiment
    Comment_CRUD -- "Simpan/Update/Hapus" --> PostgreSQL

    %% Alur Data - Search & Sort
    User -- "Search/Sort" --> Algorithms
```
...
1. **Client Tier (Frontend - React/TypeScript/Vite)**
   Bertugas sebagai lapisan interaksi pengguna. Menampilkan antarmuka yang sangat responsif (*Single Page Application*) dan merender representasi grafis (seperti *Pie Chart* dan *Keyword Cloud*) berdasarkan agregasi data (*Data Aggregation*) dari layanan Backend. Juga mengimplementasikan algoritma pencarian dan pengurutan langsung di browser (*client-side*).
...
4. **Machine Learning Tier (Backend - Python/FastAPI)**
   Lapisan komputasi AI. Python dipilih karena ekosistem pemrosesan bahasa alaminya (*NLP*) yang matang. Memiliki 2 jalur analisis:
   - **Jalur ML (Otomatis):** Worker di latar belakang menarik data dari Redis, menggunakan model bahasa (Transformers BERT) untuk menghitung sentimen setiap kalimat, meringkas topik, dan mengompilasi kesimpulan akhir.
   - **Jalur Keyword (Manual):** Endpoint CRUD untuk komentar manual menggunakan fungsi `analyze_sentiment_keyword()` yang mencocokkan teks dengan daftar kata kunci positif/negatif.

5. **Data Tier (PostgreSQL)**
   Sistem Manajemen Basis Data Relasional (*RDBMS*). Bertugas untuk merekam secara permanen hasil analisis kecerdasan buatan dan komentar manual, sehingga laporan historis dapat ditampilkan berulang kali tanpa perlu mengirim kueri analisis ulang.

## 4. Alur Proses Aplikasi (Data Flow)

### 4.1 Alur Analisis YouTube (End-to-End Pipeline)
...
6. **Penarikan Hasil (*Data Fetching*):** *Frontend*, yang menyadari bahwa proses telah rampung, mengirimkan *GET Request* ke API Python. Data dikirimkan kembali ke *Frontend* untuk dirender pada *Dashboard*.

### 4.2 Alur Komentar Manual (CRUD + Keyword Sentiment)

1. **Input Pengguna:** Pengguna mengisi form nama dan teks komentar di `CommentSection.tsx`.
2. **Analisis Keyword:** Server Python menerima teks dan menjalankan `analyze_sentiment_keyword()`.
3. **Simpan ke Database:** Komentar dan hasil sentimen disimpan ke PostgreSQL.
4. **Update UI:** React state lokal diperbarui langsung tanpa polling.

### 4.3 Alur Pencarian & Pengurutan (Client-Side)

1. **Pencarian:** Pengguna mengetik kata kunci → `algorithms.ts` menjalankan Sequential atau Binary Search.
2. **Pengurutan:** Pengguna memilih mode sort → `algorithms.ts` menjalankan Selection atau Insertion Sort.
3. **Tidak ada komunikasi ke backend** — semua diproses di browser.
...
- **Dual Sentiment Engine:** Komentar dari YouTube dianalisis dengan model ML canggih (Transformers), sedangkan komentar manual menggunakan keyword matching sederhana — sesuai dengan persyaratan Tugas Besar yang meminta analisis berbasis kata kunci.

## 6. Checklist Ketentuan Tugas Besar

| # | Ketentuan | Status | File Implementasi |
| --- | ----------- | -------- | ------------------- |
| a | CRUD Komentar (Tambah, Ubah, Hapus) | ✅ | `analysis.py` (endpoints), `CommentSection.tsx` (UI), `crud.py` (DB) |
